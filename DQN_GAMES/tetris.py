...
LIGHTCOLORS = (GRAY, LIGHTGREEN, LIGHTRED, LIGHTYELLOW)
# COLORS = (GREEN)
# LIGHTCOLORS = (GREEN)
...
class GameState:
    def __init__(self):
        ...
        self.weigth_height = 0.75
        self.weight_bumpiness = 0.5
        self.weight_hole = 1.5
    ...
    def frame_step(self,input):
        ...
        if self.fallingPiece == None:
            ...
            if not self.isValidPosition():
                ...
                print('-----------------------')
                print('Score on this game: %d' %self.score)
                print('-----------------------')
                terminal = True
        ...
        if not self.isValidPosition(adjY=1):
            ...
            # print(self.hole - self.getHole())
            self.lines += cleared
            self.total_lines += cleared
            self.getHole()

            height_change = (self.height - self.getHeight())
            bumpiness_change = (self.bumpiness - self.getBumpiness())
            hole_change = (self.hole - self.getHole())

            reward = self.weigth_height * height_change + self.weight_bumpiness * bumpiness_change + self.weight_hole * hole_change
            # print(reward)
            self.height = self.getHeight()
            self.bumpiness = self.getBumpiness() 
            self.hole = self.getHole()
    ...
    def getBumpiness(self):
        ...
        for i in range(0, BOARDWIDTH):
            ...
            for j in range(0,BOARDHEIGHT):
                if self.board[i][j] != '.' and check_first == 0:
                    sum_height += (BOARDHEIGHT - j)
                    check_first = 1
        average_height = sum_height / BOARDWIDTH
        ...
        for i in range(0, BOARDWIDTH):
            ...
            for j in range(0,BOARDHEIGHT):
                if self.board[i][j] != '.' and check_first == 0:
                    sum_bumpiness += abs((BOARDHEIGHT - j) - average_height)
                    check_first = 1
                count_height += 1
                if count_height == 20 and check_first == 0:
                    # print('!!!!!!!!!!!!!')
                    sum_bumpiness += abs(average_height)
        return sum_bumpiness / BOARDWIDTH
    ...
    def getHole(self):
        ...
        for i in range(0, BOARDWIDTH):
            ...
            for j in range(0, BOARDHEIGHT):
                if self.board[i][j] != '.' and check_first == 0:
                    count_hole = self.board[i][j:].count('.')
                    sum_hole += count_hole
                    check_first = 1
                    # print(count_hole)
                else:
    ...
    def getNewPiece(self):
        ...
        newPiece = {'shape': shape,
                    ...
                    'color': 0}#random.randint(0, len(COLORS)-1)}
        return newPiece
