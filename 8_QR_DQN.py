...
class QR_DQN:
	def __init__(self):
		...
		self.second_dense = [self.first_dense[1], self.Num_action * self.Num_quantile]
	...
	def network(self, network_name):
		...
		logits_reshape = tf.reshape(logits, [-1, self.Num_action, self.Num_quantile])
		Q_action = tf.reduce_sum(tf.multiply(1/self.Num_quantile, logits_reshape), axis = 2)

		return x_image, Q_action, logits_reshape
	...
	def loss_and_train(self):
		...
		max_tau = (2*(self.Num_quantile-1)+3)/(2*self.Num_quantile)
		tau = tf.reshape (tf.range(min_tau, max_tau, 1/self.Num_quantile), [1, self.Num_quantile])
		inv_tau = 1.0 - tau 
