...
class Curiosity_DQN:
	...
	def ICM(self):
		...
		r_i = (self.nu * 0.5) * tf.reduce_mean(tf.square(tf.subtract(forward_fc2, s_next_conv4_flat)), axis = 1)

		Lf = tf.reduce_mean(tf.reduce_sum((0.5) * tf.square(tf.subtract(forward_fc2, s_next_conv4_flat)), axis = 1))
		...
		Li = tf.reduce_mean(tf.reduce_sum(tf.square(tf.subtract(a_t, inverse_fc2)), axis = 1))
