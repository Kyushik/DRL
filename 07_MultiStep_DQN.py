		...
		if len(self.state_list) >= self.n_step:
			...
			if terminal:
				for i in range(self.n_step):
					reward_sum = 0
					for count, j in enumerate(range(i,self.n_step)):
						reward_sum += (self.gamma**count) * self.reward_list[j]

					if i == self.n_step - 1:
						self.replay_memory.append([self.state_list[i], self.action_list[i], reward_sum, next_state, True, self.n_step-i])
					else: 
						self.replay_memory.append([self.state_list[i], self.action_list[i], reward_sum, next_state, False, self.n_step-i])
			else:
				reward_sum = 0
				for i in range(len(self.reward_list)):
					reward_sum += (self.gamma**i) * self.reward_list[i]
				self.replay_memory.append([self.state_list[0], self.action_list[0], reward_sum, next_state, terminal, self.n_step])
		########################################################################################
	...
	def train(self, replay_memory):
		...
		for i in range(len(minibatch)):
			if terminal_batch[i] == True:
				...
				################################### Multi-step ###################################
				y_batch.append(reward_batch[i] + (self.gamma ** nstep_batch[i]) * np.max(Q_batch[i]))
				##################################################################################
