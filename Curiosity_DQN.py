...
class Curiosity_DQN:
	def __init__(self):
		...
		self.s_current, self.s_next, self.a_t, self.Lf, self.Li, self.r_i = self.ICM()
		self.train_step, self.action_target, self.y_target, self.loss_train, self.Q_loss = self.loss_and_train()
	...
	def main(self):
		...
		while True:
			...
			r_i = self.sess.run(self.r_i, feed_dict = {self.s_current: [stacked_state],
													   self.s_next: [stacked_next_state],
													   self.a_t: [action]})
	...
	def ICM(self):
		...
		with tf.variable_scope('curiosity'):
			...
			# Densely connect layer variables (Forward model)
			for_w_fc1 = self.weight_variable('_for_w_fc1', [800 + self.Num_action, 256])
			for_b_fc1 = self.bias_variable('_for_b_fc1',[256])

			for_w_fc2 = self.weight_variable('_for_w_fc2', [256, 800])
			for_b_fc2 = self.bias_variable('_for_b_fc2',[800])

			# Densely connect layer variables (Inverse model)
			inv_w_fc1 = self.weight_variable('_inv_w_fc1', [2*800, 256])
			inv_b_fc1 = self.bias_variable('_inv_b_fc1',[256])

			inv_w_fc2 = self.weight_variable('_inv_w_fc2', [256, self.Num_action])
			inv_b_fc2 = self.bias_variable('_inv_b_fc2',[self.Num_action])
		...
		s_conv4_flat = tf.reshape(s_conv4, [-1, 800])
		...
		s_next_conv4_flat = tf.reshape(s_next_conv4, [-1, 800])
		...
		forward_fc1 = tf.nn.relu(tf.matmul(input_forward, for_w_fc1)+for_b_fc1)
		forward_fc2 = tf.matmul(forward_fc1, for_w_fc2)+for_b_fc2

		Lf = (0.5) * tf.square(tf.norm(tf.subtract(forward_fc2, s_next_conv4_flat), ord = 2))
		...
		inverse_fc1 = tf.nn.relu(tf.matmul(input_inverse, inv_w_fc1)+inv_b_fc1)
		inverse_fc2 = tf.nn.softmax(tf.matmul(inverse_fc1, inv_w_fc2)+inv_b_fc2)

		Li = tf.norm(tf.subtract(a_t, inverse_fc2), ord = 1)

		return s_current, s_next, a_t, Lf, Li, r_i

	def loss_and_train(self):
		# Loss function and Train
		action_target = tf.placeholder(tf.float32, shape = [None, self.Num_action])
		y_target = tf.placeholder(tf.float32, shape = [None])
		Q_loss = tf.placeholder(tf.float32, shape = [None])
		...
		Loss = - self.lamb * tf.reduce_mean(Q_loss) + (self.beta * self.Lf) + ((1-self.beta) * self.Li)
		train_step = tf.train.AdamOptimizer(learning_rate = self.learning_rate).minimize(Loss)

		return train_step, action_target, y_target, Loss, Q_loss
	...
	def train(self, replay_memory):
		...
		# Get y_prediction
		y_batch = []
		Q_batch = self.output_target.eval(feed_dict = {self.input_target: state_batch})
		Q_next_batch = self.output_target.eval(feed_dict = {self.input_target: next_state_batch})
		...
		for i in range(len(minibatch)):
			if terminal_batch[i] == True:
				...
				y_batch.append(reward_batch[i] + self.gamma * np.max(Q_next_batch[i]))
